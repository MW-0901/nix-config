...
in
{
  ...
  system.activationScripts.wpilibDockerBuild = lib.stringAfter [ "users" "groups" ] ''
    echo "Destroying previous docker container..."
    ${pkgs.docker}/bin/docker ps -a | ${pkgs.gnugrep}/bin/grep frc-dev | ${pkgs.gawk}/bin/awk '{print $1}' | ${pkgs.findutils}/bin/xargs -r ${pkgs.docker}/bin/docker rm -f || true
